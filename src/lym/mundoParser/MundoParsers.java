...
public class MundoParsers {
	...
	*/
	...
	public Logo getParserLogo(){
		return new Logo(System.in);
	}
	...
	public String  procesarCadena(String texto) {
		...
		*/
		if(parsers.get(currentParser).equals("Parser Logo")){
			Logo parserLogo = getParserLogo();
			parserLogo.ReInit(new java.io.StringReader(texto));
			try {
		    	parserLogo.esCorrecto();
		    	resp = new String("OK    \n");
		    }catch (Exception e) {
		        resp = new String ("Error de Sintaxis: "+e.getMessage());
		     } catch (Error e) {
